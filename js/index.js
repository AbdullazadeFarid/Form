...
document.getElementById("form").addEventListener("submit" , function(e){  //get formu ID ile cagir ve bunda olan (e)eventi dinle(EVENT HER HANSİ BİR HADİSEYE DEYİLİR) , eger submit olarsa(yeni form da yerlesen button type submute click edildikde)
    ...
    var email = document.getElementById("email").value //biz value komekliyi ile ID si email olan inputun icinde yazilanlari goruruk
    ...
    if(email.trim()=== ""){  //trim yeni sag ve sol hissedeki space(probeli silir) ve bir deyer kimi qebul etmir
    ...
    if(!email.includes("@")){    //burada qeyd edirikki @ isaresi olmadan melumattdar qeyde alinmasin
